...
# MAGIC CREATE TABLE if not exists `demo_turbine`.`turbine_bronze` USING delta LOCATION 'dbfs:/mnt/field-demos/manufacturing/iot_turbine/bronze';
# MAGIC CREATE TABLE if not exists `demo_turbine`.`turbine_silver` USING delta LOCATION 'dbfs:/mnt/field-demos/manufacturing/iot_turbine/silver';
# MAGIC CREATE TABLE if not exists `demo_turbine`.`turbine_gold`   USING delta LOCATION 'dbfs:/mnt/field-demos/manufacturing/iot_turbine/gold' ;
# MAGIC 
# MAGIC CREATE TABLE if not exists `demo_turbine`.`turbine_power_prediction` USING delta LOCATION 'dbfs:/mnt/field-demos/manufacturing/iot_turbine/prediction';
# MAGIC CREATE TABLE if not exists `demo_turbine`.`turbine_power_bronze`     USING delta LOCATION 'dbfs:/mnt/field-demos/manufacturing/iot_turbine/power';
...
# MAGIC 
# MAGIC Be smart, fake your data in your SQL select statement for the counters to save some time. (`SELECT 35 AS turbine_par_efficiency` is a really good query to build such dashboard)
